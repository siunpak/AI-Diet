...
# %%
from util import get_action, get_score_pandas, get_reward_ver2, get_score_vector, make_synthetic_target, changing_buffer
...
class Sequence_Generator(tf.keras.Model):
    def __init__(self, food_dict, nutrient_data, incidence_data, candidate_data, **kwargs):
        super().__init__(self)
        ...
        self.buffer_change_num = kwargs['buffer_change_number']
    ...
    def train(self, x, x_update):
        ...
        with tf.GradientTape() as tape:
            ...
            if self.synthetic_target_buffer == True:
                synthetic_target = make_synthetic_target(x, beta_score, pred_seqs, self.nutrient_data)
                
                synthetic_target = changing_buffer(synthetic_target, self.nutrient_data, self.buffer_change_num, self.candidate_data)
