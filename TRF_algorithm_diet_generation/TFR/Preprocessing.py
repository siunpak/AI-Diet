...
feature_data = pd.read_csv('./data/nutrient_data/nutrition_db_AI_camp(6~8).csv', encoding = 'cp949', index_col = 0)

## -- (2) Load diet sequence dataset
# if you want to load the diet data, including breafast meal, which is of 17 length.

diet_data = pd.read_csv('./data/diet_data/diet68_boy_morning.csv', encoding = 'cp949')
diet_data_for_incidence = pd.read_csv('./data/incidence_data/diet_boy_morning_incidence.csv', encoding = 'cp949')


with open("candi_foods_68_morning.pkl", "rb") as file:
    candidate_data = pickle.load(file)
...
# Define parameters required for preprocessing
inci_preprocessing_kwargs = {
    'sequence_data' : diet_data_for_incidence,
    'feature_data' : feature_data,
    'integrate' : False,    
...
# Preprocess diet data
preprocess_diets = diet_sequence_preprocessor(**inci_preprocessing_kwargs)
inci_diet_data = preprocess_diets(nutrient_data)

diet_data_for_incidence_np = food_to_token(inci_diet_data, nutrient_data, empty_delete = True, num_empty = 3)


# Mapping from diet_data_np to binary incidence format
incidence_data = diet_to_incidence(diet_data_for_incidence_np, food_dict)
...
buffer_size = 60
buffer_update = 10
buffer_change_num = 20  ## Buffer 내 임의로 얼마나 바꿀 것인지
num_epochs = 30000
lr = 5*1e-4
...
kwargs = {
    ...
    'buffer_change_number': buffer_change_num,
