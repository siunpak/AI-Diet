...
def createDir(root_dir, params):
    ...
    save_dir = target_dir
    print(save_dir)#
    ## + '--'.join([i + "=" + str(params[i]) for i in list(params) if i not in ['fully-connected_layer', 'attention', 'num_tokens', 'token_dict']])
...
def make_synthetic_target(x, beta_score, pred_seqs, nutrient_data):
    ...
    if len(candidates_idx) > 0:
        ...
        nutri_exp = np.apply_along_axis(get_score_vector, axis = 1, arr = np.array(experiences), nutrient_data = nutrient_data)
        score_exp = np.apply_along_axis(get_reward_ver2, axis = 1, arr = nutri_exp, done = 0)[:, 0]
        new_idx = np.where(score_exp >= thres_score)[0]
        new_exp = experiences[new_idx, :]
        
        target_buffer[candidates_idx[new_idx], :] = new_exp
...
def changing_buffer(synthetic_target, nutrient_data, buffer_change_num, candidate_data):
    
    import random

    
    random_idx = random.sample(range(len(synthetic_target)), buffer_change_num)
    change_buffer = synthetic_target[random_idx, :]
    # 시작 - 간편식1- 간편식2 - 간편식3 - 간식3 - 간식3 - 밥 - 국 - 주찬 - 부찬 - 김치 - 간식1 - 간식2 - 밥 - 국 - 주찬 - 부찬 - 김치 - 종료
    non_rice_sequence = [1,2,3,4,5,7,8,9,10,11,12,14,15,16,17]
    
    for i in range(len(change_buffer)):
        sequence_idx = random.sample(non_rice_sequence, 3)
        food_idx = change_buffer[i][sequence_idx]
        foods = list(nutrient_data.iloc[food_idx]['name'])
        ...
        for food in foods:
            if (food == '종료') or (food == 'Empty') or (food == 'empty'):
                ...
                change_menu = random.sample(candidate_data[food], 1)
                
            change_foods.append(nutrient_data[nutrient_data.name == change_menu[0]].index[0])
        
        if change_foods not in change_buffer[i]:
            change_buffer[i][sequence_idx] = change_foods
                
    
    synthetic_target[random_idx, :] = change_buffer
...
def update_dataset(epoch, BATCH_SIZE, target_buffer_update, target_buffer, tf_dataset_update, x, food_dict, nutrient_data):
    ...
    if (epoch + 1) % target_buffer_update == 0:
        ...
        ## 각 배치별로 target_buffer에서 하나의 buffer_memory를 샘플링한 후 new_diet_data_np에 stack하기
...
def get_reward_ver2(nutrient_state, done):
    ...
    # 영양보상 1. 총 열량 (kcal)
    if total_calorie >= 1530 and total_calorie <= 1870:   ### 6~8세 남자
    #if total_calorie >= 1350 and total_calorie <= 1650:  ### 6~8세 여자
    #if total_calorie >= 1800 and total_calorie <= 2200:  ### 9~12세 남자
    #if total_calorie >= 1620 and total_calorie <= 1980:  ### 9~12세 여자
    
        nutrient_reward += 1
        nutrient_reward_set[0] +=1

    # 영양보상 2. 단백질 (g)
    if total_p_gram >= 26.25 :   ### 6~8세 남자 / 여자
    #if total_p_gram >= 37.5 :  ### 9~12세 남자
    #if total_p_gram >= 33.75 :  ### 9~12세 여자
        nutrient_reward += 1
        nutrient_reward_set[1] +=1

    # 영양보상 3. 총 식이섬유 (g)
    if total_dietary >= 13 and total_dietary <= 25:   ### 6~8세 남자
    #if total_dietary >= 13 and total_dietary <= 20:  ### 6~8세 여자
    #if total dietary >= 16 and total_dietary <= 25:  ### 9~12세 남자 / 여자
    
        nutrient_reward += 1
        nutrient_reward_set[2] +=1

    # 영양보상 4. 비타민 A (mugRAE)
    if total_vitaA >= 337.5 and total_vitaA <= 562.5:   ### 6~8세 남자
    #if total_vitaA >= 300 and total_vitaA <= 500:      ### 6~8세 여자
    #if total_vitaA >= 450 and total_vitaA <= 750:      ### 9~12세 남자
    #if total_vitaA >= 412.5 and total_vitaA <= 687.5:  ### 9~12세 여자

        nutrient_reward += 1
        nutrient_reward_set[3] +=1

    # 영양보상 5. 비타민 C (mg)
    if total_vitaC >= 37.5 and total_vitaC <= 62.5:   ### 6~8세 남자 / 여자
   #if total_vitaC >= 52.5 and total_vitaC <= 87.5:   ### 9~12세 남자 / 여자
        nutrient_reward += 1
        nutrient_reward_set[4] +=1

    # 영양보상 6. 비타민 B1 (mg)
    if total_vitaB1 >= 0.525 and total_vitaB1 <= 0.875:   ### 6~8세 남자 / 여자 
    #if total_vitaB1 >= 0.675 and total_vitaB1 <= 1.125:  ### 9~12세 남자 / 여자
        nutrient_reward += 1
        nutrient_reward_set[5] +=1

    # 영양보상 7. 비타민 B2 (mg)
    if total_vitaB2 >= 0.675 and total_vitaB2 <= 1.125:   ### 6~8세 남자
    #if total_vitaB2 >= 0.6 and total_vitaB2 <= 1.0 :     ### 6~8세 여자
    #if total_vitaB2 >= 0.825 and total_vitaB2 <= 1.375 : ### 9~12세 남자
    #if total_vitaB2 >= 0.75 and total_vitaB2 <= 1.25 :  ### 9~12세 여자
    
        nutrient_reward += 1
        nutrient_reward_set[6] +=1

    # 영양보상 8. 칼슘 (mg) 
    if total_calcium >= 525 and total_calcium <= 875:   ### 6~8세 남자 / 여자
    #if total_calcium >= 600 and total_calcium <= 1000: ### 9~12세 남자 / 여자
        nutrient_reward += 1
        nutrient_reward_set[7] +=1

    # 영양보상 9. 철 (mg)
    if total_iron >= 6.75 and total_iron <= 11.16:   ### 6~8세 남자 / 여자
    #if total_iron >= 8.25 and total_iron <= 13.75:  ### 9~12세 남자
    #if total_iron >= 7.5 and total_iron <= 12.5:  ### 9~12세 여자
        nutrient_reward += 1
        nutrient_reward_set[8] +=1

    # 영양보상 10. 나트륨 (mg)
    if total_sodium <= 1900:   ### 6~8세 남자 / 여자
    #if total_sodium <= 2300:   ### 9~12세 남자 / 여자
        nutrient_reward += 1
        nutrient_reward_set[9] +=1

    # 영양보상 11. 리놀레산 (g)
    if total_lino <= 11250:  ### 6~8세 남자
    #if total_lino <= 8750:  ### 6~8세 여자
    #if total_lino <= 11875:  ### 9~12세 남자
    #if total_lino <= 11250:  ### 9~12세 여자
        nutrient_reward += 1
        nutrient_reward_set[10] +=1

    # 영양보상 12. 알파 리놀렌산 (g)
    if total_alpha <= 1375:   ### 6~8세 남자
    #if total_alpha <= 1000:  ### 6~8세 여자
    #if total_alpha <= 1625:  ### 9~12세 남자
    #if total_alpha <= 1375:  ### 9~12세 여자
        nutrient_reward += 1
...
# 한영번역기
#translate_book = pd.read_csv('translated_menu_name (수정).csv', encoding='CP949')
...
def get_most_change_sequence(real_seqs, gen_seqs, top_n):
    ...
    for i in range(gen_seqs.shape[0]):
        dist_by_pair = np.append(dist_by_pair, distance.jaccard(gen_seqs[i], real_seqs[i]))
